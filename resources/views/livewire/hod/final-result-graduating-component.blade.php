                    ...
                    <div class="row" wire:key="{{ $bos_log->id }}">
                        <div class="col-md">
                            <a href="JavaScript:void();" class="btn btn-sm btn-danger btn-block"
                                onClick="MM_openBrWindow('{{ route('hod.results.graduating-semester-result', [
                                ...
                                Print Graduating Spreadsheet - {{ $bos_log->presentation }}
                            </a>
                        </div>
                        <div class="col-md">
                            <a href="JavaScript:void();" class="btn btn-sm btn-danger btn-block"
                                onClick="MM_openBrWindow('{{ route('hod.results.graduating-running-list', [
                                    'encoded_session' => base64_encode($bos_log->session),
                                    'set' => $bos_log->adm_year,
                                    'level_id' => $bos_log->level_id,
                                    'prog_type_id' => $bos_log->prog_type_id,
                                ...
                                Print Graduating Running List - {{ $bos_log->presentation }}
                            </a>
                        </div>
                    </div>
                @endforeach
