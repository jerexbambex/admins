...
class DeadStudentsComponent extends Component
{
    ...
    public function addPenalty($data)
    {
        if ($data) {
            ...
            })->orWhere('std_logid', $data['log_id'])->update(['status' => 'dead'])) return true;
        }
        return false;
    }
    ...
    public function submit()
    {
        ...
        if ($this->validated) {
            ...
            if ($this->addPenalty($data)) {
                session()->flash('success_toast', "Student status set to dead!");
                $this->reset();
                $this->mount();
            } else {
    ...
    public function render()
    {
        ...
        return view('livewire.penalties.dead-students-component', compact('sessions', 'penalties'));
    }
}
